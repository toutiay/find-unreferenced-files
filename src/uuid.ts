...
function compressUuid(uuid: string, min?: boolean) {
...
function compressHex(hexString: string, reservedHeadLength: number) {
...
function decompressUuid(str: string) {
    if (str.length === 23) {
        // decode base64
        var hexChars = [];
...
export function encodeUuid(uuid: string, min?: boolean): string {
    return compressUuid(uuid, min);
}

export function decodeUuid(str: string): string {
